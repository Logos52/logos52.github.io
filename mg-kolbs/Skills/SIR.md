...
SIR (Spaced Interleaved Retrieval) is [[wiki/Techniques/Spaced Interleaved Retrieval|a retrieval practice method]] that distributes recall attempts across time (spaced) and across multiple topics within a session (interleaved). The goal is to force the brain to reconstruct rather than simply recognise.
