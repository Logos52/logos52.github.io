...
- `.kb-ac-domain` is `DOMAIN_LABELS` (`src/lib/types.ts`). Vibe Coding's chip reads `Agentic`, not the folder name `AI & Agentic Systems`. The note URL still uses `slugifyFilePath` (`AI--and--Agentic-Systems`, spaces around `&`). Assert the chip text and `h1.kb-note-title`, not a hyphen count in the address bar.
