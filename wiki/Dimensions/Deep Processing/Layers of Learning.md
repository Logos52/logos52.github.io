...
1. **Find the logic first.** Before engaging with details, skim the source for the organizing principles: what are the main relationships, mechanisms, or causal chains? This is the most cognitively expensive pass — it requires evaluation, not just reading — but it establishes the frame that makes every later pass cheaper.

2. **Add concepts to the frame.** Once the logic layer exists, the major concepts can be placed into it. Each concept now has a location: it sits in relation to the logic, rather than floating in isolation.

3. **Add details last.** Details attach to concepts. Because the concept layer already exists, details have a place to land. Information that would have required memorization without context becomes obvious or near-obvious once the surrounding structure is in place.
...
[[wiki/Dimensions/Deep Processing/Prestudy|Prestudy]] is the first-pass implementation of layered learning at the topic level: survey the whole topic to find the logical frame before the main learning event. The prestudy pass builds the logic layer; the main learning events fill in concepts and details.
