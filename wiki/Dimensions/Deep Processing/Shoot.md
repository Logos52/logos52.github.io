...
> Part of [[wiki/Dimensions/Deep Processing|Deep Processing]], the parent dimension for encoding work.

Shoot is reading the material to answer questions written before it was opened, with every answer going onto a map of your own that is allowed to keep changing. It is the second step of the [[wiki/Dimensions/Deep Processing/Bear Hunter System|Bear Hunter System]]: [[wiki/Dimensions/Deep Processing/Aim|Aim]] wrote the questions, and Shoot spends the material answering them, not collecting from it. That is the point where the material becomes useful: the curiosity Aim built gets satisfied, and a working map takes shape while the learning is still happening.

Learned as a drill, Shoot replaces the green light of the traffic light drill, the part of that drill that answered the questions, and replaces the old notes. From here the drill is dropped and Aim and Shoot run alone. A set of note rules, added one at a time, keeps the old habit of reading front to back and writing sentences out of the pass. The pass is meant to feel effortful and uncertain, it runs before, during, and after each class, and it has a one-sentence bar.

## Core takeaways

- Read the material to answer the questions Aim wrote, and put every answer on a map. The material's order is followed only where it is the best structure on offer; the sequence belongs to the reader.
- An answer counts when it is logical, so its origin is visible and it can be rebuilt, and interconnected, so it arrives attached to what it relates to. Across 55 studies, building structures of labelled relations beat conventional study by about half a standard deviation.
- Detail waits until the structure it belongs to exists. Met after the concept layer exists, a detail tends to stop needing to be held, because it follows from the logic around it.
- The note rules, added one at a time, block the old habit: visual, processed, relational, freehand, reflective, efficient. A map that follows the textbook's order is the sign something went wrong.
- Effort and confusion are the brain working. Drowsiness, when sleep is not the cause, means the processing has stopped. Certainty comes late, and early certainty is the illusion.
- Shoot runs before the class if there is time, during it in short lines, and after it to finish. The bar is that Aim and Shoot connect without a seam, with high chunking and accurate relationships, at moderately low effort.

## Before you start

The Aim bar has to be in place. A concept list turns into high-quality questions that find chunks, sub-chunks, and the relationships between them, every time. Focus sets up easily in any environment. Spaced interleaved retrieval is near unconscious competence, with several interleaving methods and consistent spacing. Non-linear notes are getting comfortable. Those are what Shoot stacks on. It is one new technique that folds several earlier ones into one pass.
...
Two things get replaced and three keep running. The green light of the traffic light drill, answering the questions, is replaced by Shoot, so the drill is now gone entirely and the two passes run alone. The old notes, collecting on one side of the page and processing on the other, are replaced by the map; Cornell notes are the overhyped version of collect and process. Aim continues as it was. Maintaining focus and BEDS-M start turning into habits. Scheduling basics and spaced interleaved retrieval should be second nature by now.

## The step

**The questions sit beside the material, and the reading is done to answer them.** Each thing the material offers gets one of four treatments: it answers a question; it goes under the chunk it belongs to; it adds a relation, if it changes the map; or it gets marked incomplete. Three questions run in the head for each thing the material offers: what is it, why is it important, how does it relate. The answer to the second is usually a link to something already on the map, and that link is what gets drawn. The material's order is followed only where it is the best structure on offer. The sequence belongs to the reader, which is [[wiki/Dimensions/Deep Processing/Order Control|Order Control]] running inside the step.
...
The note rules hold the notes to the pass. They are added one at a time, because a rigid linear habit does not change in a day. Visual: fewer words, rarely a sentence, almost never a paragraph. Doodles, symbols, and shorthand do the carrying, because re-drawing a paragraph as a doodle is the processing. Processed: nothing goes on the page that did not pass through the three questions first. Copying is fast and produces no learning. Relational: relationships on the page, not items, since an isolated fact is the one the brain drops. Freehand: by hand on an infinite canvas with a stylus, because typing is single-minded and represents relationships badly. Reflective: in your order, the one Aim chose, and if the notes follow the textbook's order something went wrong. Efficient: after a block, ask whether you kept it and can use it. Forgetting more than 30 percent of what you wrote means one of the earlier rules failed.

The map itself is a technical skill. Relationships are planned before content goes down, first maps are messy with crossing lines, and redoing them is part of the system. Emphasise the flow that matters so the eye knows where to go.

## The map stays provisional

**The map is the product, and it is never finished while the reading runs.** Concepts move, chunks get renamed, an overloaded area splits, thin areas merge, missing relations get drawn in, and details that stopped mattering come off. A map still matching its first guess at the end of a session is the thing to notice, because understanding that improved would have moved something. This is also why the material's order was never the default: a structure borrowed whole is a map that cannot move.

## What waits, and what does not

**Detail waits until the structure it belongs to exists.** Encoded early it costs more than it would to miss: met before its surroundings, it sits isolated, harder to retain, headed for memorization instead of inference. Met after the concept layer exists, the same detail tends to stop needing to be held; what would not stick alone follows from the logic around it. [[wiki/Dimensions/Deep Processing/Layers of Learning|Layers of Learning]] owns the theory behind that ordering.

The exception is immediate: a detail that clarifies a relation gets captured on the spot, and so does one that heads off a misunderstanding about a chunk. Past the exception, a detail earns its place by doing work: explaining a relation, supporting a chunk, clarifying an example, preparing for assessment, heading off a likely forgetting, exposing a gap. Appearing in the material is not one of those jobs.

What slips past is not lost: missed details surface in later retrieval, in the cut-down pass, or on the next pass through the material. The system runs in layers, each catching what the one before missed. Not every question gets a clean answer either, and the attempt is still where the learning happens, because the question drives the search for relations; one too advanced for this pass gets marked and left, not forced.
...
- The map no longer matches the first guess, and it does not follow the material's order.
- The main relations can be said out loud with the material closed.
- Unresolved questions carry a visible mark.

## How it feels

Effort during the step is the expected condition. The work is supposed to feel like figuring something out, and [[wiki/Concepts/Cognitive Load & What Mental Effort Is Trying to Cue|Cognitive Load & What Mental Effort Is Trying to Cue]] reads what that effort signals. Effort, confusion, and the struggle to work something out are the signs the brain is working. Difficulty tracks learning, and the effect holds across ages and subjects. The opposite signal is just as usable: study easy enough to produce drowsiness, when sleep is not the cause, is a technique reading. The processing has stopped.
...
The pass also withholds certainty for a long time. The certainty that rereading and copying hand out early is an illusion. It is where inconsistent results, frustration, and exam anxiety come from.

A learner who does not find the pass difficult is the one to worry about. It usually means the technique is not being applied to the standard. Time spent building a backbone, working out concepts, arranging them in space, and expressing them without words is all learning. Time spent reading, writing, and memorising with no relationship in view is the time wasted. Once the pass is a habit it runs on its own and is faster than the old method was. The hardest part is believing that and committing to it.

## When to shoot

Shoot happens before, during, and after the main learning event. If there is time after the Aim, shoot the most basic concepts and relationships before the class. Continue the aim-shoot cycle in the class and finish the shoot there. If there was no time before, shoot after. The class becomes one more resource. In the standard week, Shoot runs around each class from Monday to Friday, and [[wiki/Dimensions/Deep Processing/Skin|Skin]], the next pass, takes the map at the end of the week.
...
**Trouble in this step usually traces back to the questions.** When the questions were good, Shoot is the easy half: direction exists, the work is answering, details have somewhere to go, the map moves, weak chunks show themselves. When it is hard, either the questions were thin or the old linear pass took over as the material opened. The signs are on the page: a map mirroring the material's table of contents, full sentences accumulating, chunks that never move because moving them feels like wasting the drawing, every detail at equal weight, linear blocks with arrows added afterwards, scattered work with nothing holding it together. Catching any of this mid-session is [[wiki/Dimensions/Self-Regulation|Self-Regulation]]'s work, the control layer watching while the session runs.

| The session shows | The move |
|---|---|
| Full sentences accumulating | One Aim question gets answered from memory, material closed. |
| Detail piling up | Each detail names the chunk it serves; one with no chunk parks or goes. |
| A chaotic map | The map rebuilds from its three to five largest chunks. |
| Relations that stay vague | The connecting question runs on one pair at a time: how this relates to that. |
| An Aim that was too thin | The session pauses and re-Aims before more reading happens. |

The quit signal is one specific test. Main relations that still cannot be said from memory at the end of a session put the fault in the questions, and the next move is re-Aiming rather than shooting harder.

Continuous writing is the standing temptation: it feels productive, and it removes the pressure to process. Deciding what a passage amounts to is the processing, so notes carry conclusions and transcription stays in the material. What the note research supports is narrower than the way it gets quoted: verbatim capture is the weak form, and typing makes verbatim drift easy. Pasting the textbook's diagram is the same move, because pasting removes the processing. The non-linear form itself takes practice, and a failed first attempt is usually flawed form rather than a weak method; [[wiki/Dimensions/Deep Processing/Mindmaps|Non-Linear Note-Making]] owns the full treatment. Collecting text is the cheap move here, the one [[wiki/Concepts/The Shortcut Problem|The Shortcut Problem]] names: the session gets discharged while the thing it was for gets skipped. A Shoot can also look correct and still be copying, right format, moved boxes, nothing processed, the case [[wiki/Dimensions/Self-Regulation/The Technique Is Only as Good as the Thinking It Produces|The Technique Is Only as Good as the Thinking It Produces]] covers.

The cost comes early: slower per pass than transcription, with the feeling of coverage withheld. Nothing looks finished until the relations hold, so certainty arrives late. That cost falls as the technique stops needing attention, and what the fast pass buys instead is a sense of completion that does not track what was built. The boundary sits at material with no relational structure to find: conventions, arbitrary curriculum facts, vocabulary. Those belong in memorization tools, and forcing them onto the map is one of the ways a map gets ruined.

## The loop

With practice the two steps run as one motion: answering produces the next question, and it gets aimed and shot without a seam. The cycle cannot be forced. It comes from competence in each step separately, and anyone not yet cycling keeps running them apart. Many of a topic's questions do not exist until answering starts; that is why the loop exists at all. What a session leaves is a working map, questions answered, relations clear, details attached, the unresolved marked, ready for [[wiki/Dimensions/Deep Processing/Skin|Skin]], the pass that cuts it down to what can be rebuilt without the material.

## The bar

The bar for Shoot is one sentence. You connect the aim and shoot steps accurately and seamlessly, with high chunking and accurate relationships between chunks, and the effort this takes has become moderately low. Focus, scheduling, BEDS-M, and revision have become habits by this point, at unconscious competence. About 80 percent competence with everything taught so far is the level to move on.
...
The checkpoint looks at three things, and people slack on the first two. Kolb's cycle is run consistently and at a high level. From here on, diagnosing why results are off becomes fine-grained. A reflection that is brief, or a rule that is theoretical rather than observed, stalls progress. The fundamentals, spaced interleaved retrieval above all, are still running and were not dropped while the encoding was being learned. And the aim-and-shoot maps themselves. What the checkpoint reads in those maps, whether the chunks are importance-based, whether there are single-node chains, chunks appearing from nowhere, reverse causality, keyword lists indented the way the textbook was, is the standard, and it is on Shoot - Training II. What the map is then held to, layers, root reasons, and the map-reading checks, is on that page too. Work on the fence can move on, slowly, while the chunking is refined to the edges.

## How to practice this

1. Put the Aim questions beside the material and read to answer them. Notice the session flipping back and forth instead of running front to back.
2. Ask three questions of each thing the material offers: what is it, why is it important, how does it relate. Notice that the answer to the second is usually a link to something already on the map.
3. Add one note rule at a time. Notice whether the map follows the textbook's order; if it does, something went wrong.
4. At the end of a session, close the material and say the main relations out loud. Notice which ones cannot be said. Those put the fault in the questions, so re-Aim before reading more.
5. After a block, ask whether you kept what you wrote and can use it. Notice if more than 30 percent is gone; one of the earlier rules failed.
6. Watch how the session feels. Effort and confusion mean the brain is working. Drowsiness, when sleep is not the cause, means the processing has stopped.
...
- [[wiki/Concepts/Cognitive Load & What Mental Effort Is Trying to Cue|Cognitive Load & What Mental Effort Is Trying to Cue]]: what the effort in this step signals, and what overwhelm means when the questions were thin or detail came early.
- [[wiki/Dimensions/Self-Regulation/The Technique Is Only as Good as the Thinking It Produces|The Technique Is Only as Good as the Thinking It Produces]]: a Shoot that looks correct and is still copying.
- [[wiki/Concepts/The Shortcut Problem|The Shortcut Problem]]: collecting text as the cheap move the step displaces.
- [[wiki/Concepts/Learning Styles Myth and Multimodal Learning|Learning Styles Myth and Multimodal Learning]]: why the visual rule works on everyone.
- [[wiki/Dimensions/Self-Management/Kolbs Experiential Cycle|Kolbs Experiential Cycle]]: the check the checkpoint grades hardest.
...
- The rules say add one at a time. In what order, and how long on each before the next?
- The bar says moderately low effort. Measured how, when the pass is meant to feel effortful?
...
- The iCanStudy program by Justin Sung, learning.icanstudy.com. Ascent II: *Bear Hunter System: Shoot*. *Effective note-taking* with its three sub-lessons, Visual, then Processed, relational, freehand, then Reflective. *Beginner's mind map guide*. *Mindmap dissection*. *Barriers for shooting*. *Final checkpoint*. Private course text captured to the vault's raw folder, never republished, read in full on 2026-09-07. The note-taking lesson corrects its own left-brain right-brain claim, and that correction is followed here. Every number taken from the program is the program's own working number, not a measurement.
