/** Pure spine-graph zoom math — unit-testable without Pixi/d3. */
...
export const SPINE_MAX_FIT_SCALE = 2.8
...
export function spineHubBandBounds(
  width: number,
  height: number,
  hubCount = SPINE_HUB_BAND_COUNT,
): Bounds {
...
export function computeSpineAutoFitBounds(
  width: number,
  height: number,
  hubs: HubPlacement[] = [],
): Bounds {
  ...
  for (const hub of hubs) {
    bounds = unionBounds(
      bounds,
      labelScreenBounds(hub.simX + width / 2, hub.simY + height / 2, hub.label),
    )
  }
  return bounds
}
...
/** After fit transform, every corner of bounds should sit inside the canvas minus margin. */
export function boundsFitInViewport(
  bounds: Bounds,
  width: number,
  height: number,
  scale: number,
  ...
  return corners.every(({ x, y }) => {
    const sx = (x - midX) * scale + width / 2
    const sy = (y - midY) * scale + height / 2
