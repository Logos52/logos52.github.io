...
- No pointer to where a fact came from anywhere outside the Sources block. "The video says", "the episode cites", "in the lecture's claim", "the talk's close", and figure captions that credit the video are the same fault as a name in the body: the sentence says what is true, and the Sources block says who said it. This holds for every section of a page, the Reasoning section, the numbers, the diagrams, and the open questions alike. `scripts/source-words-check.py` sweeps the wiki for the media words and lists every hit; a hit is fixed by restating the fact, never by rewording the pointer. What stays in the body is the thing that happened in the world: a survey that asked 3,000 voters, a report with a name, a study with a sample. A bold claim in the opening names that thing right there, because the claim needs it (owner: "it was a bold claim so this one definitely requires a source in the intro"). What never appears in the body is the medium the page was compiled from, the video, the transcript, the captions, the episode. Owner, 2026-09-05: "taking 'the video' out of pages should be a general solution, not specific. no sources in anywhere except sources section." (Ruled 2026-09-05.)
...
- A reader holds about four things at once. A sentence that asks them to hold more is split. One next step per sentence. Splitting keeps every fact. (Ruled 2026-09-01.)
