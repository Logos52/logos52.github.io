...
**Authority (set 2026-06-11; May standard deleted 2026-06-12).** For wiki pages, **High-Signal Wiki Pages**. For decision documents, **High-Signal Decision Writing**. For domain-compression pages, **Condensed Pages**. For front-facing surfaces and public posts (blog, explainers, essays) on any project, **High-Signal Front-Facing Pages**. For personal pages, **The Personal Register**. For operational pages — directions, references, structures, workflows — **The Cold Open**. These six sections, plus the site-wide **Whole–Part–Whole** law directly below, are the complete standard. The earlier May rules are deleted — do not cite them; they live only in git history (bd5adc9). **Default register, ruled 2026-08-10:** going forward, wiki and personal pages are written in the blog register's voice — see §The Default Register below for scope and exemptions.
...
## The Cold Open — operational pages (2026-08-12)

Ruled 2026-08-12 across four pages in one session; human-consumption treatment at [[wiki/Writing Craft/The Cold Open|The Cold Open]]. Scope: operational pages — directions, references, structures, workflows, and position statements: pages whose reader arrives to use them. The Default Register's run-up law (2026-08-10) continues to govern posts, concept tellings, and personal pages; this section carves the operational genre out of it. The carve-out is WPW-compliant by the site law's own text — the opening whole is each genre's own form, and here the whole is the bare claim.

- **The first sentence is the operating claim at full strength.** No run-up, no scene, no history. Accepted exemplars, same day: "The research here splits into a standing half and a session half." · "Bots watch; the desk changes things." · "A language model's default voice is trained self-regard." The struck counter-sample: Standing Research Agents' first opening, a paragraph of the wiki's life story before the claim — struck in Wedge's words: "the intro is too grandiose. i want it to be smaller stakes. just go cold thesis. don't need to overthink too hard about it."
- **Small stakes, held.** The page never raises its own stakes: no before-and-after narrative, no significance announcement. What changed is stated as fact; the reader supplies the weight. "Don't overthink" is part of the ruling — a cold open is written once, plainly, and left alone.
- **The second move is application.** After the claim, the parts arrive immediately — the lanes, the rules, the pipeline. The open discharges the whole; the body spends it.
- **Bolded operative leads are the scan spine.** Sections open with the rule stated bold, mechanism following in the same breath; numbered lists carry stacked reasons, each number one complete reason. (Exemplar: Bot Operating Rules §The report is the product.)
- **Declaratives carry the register.** Rules are statements, never advice aimed at the reader. The honesty kit binds in full: price in the same breath, quit signals present, Case Against where a structure is proposed (exemplar: Grok Bot Fleet Structures §The case against A).
- **The close returns without ceremony.** The last section lands back in the opening claim advanced, one breath, no coda ("retirement is the system working").
- Everything else inherits unchanged: True Against the System (same-day strike: "an agent never writes one," false against the vault's own practice, corrected on sight), the Delete List, No Scaffolding, sources and provenance rules; em dashes stay legal in this genre.
