...
Corrections to an LLM collaborator sort into four classes by the mechanism that produces them, and the class predicts which repair will hold. A model carries no state between generations, so anything settled earlier drops out of the next rewrite unless something checks for its absence. Its generator returns to its own defaults — compress, crown one answer, take the surface reading, [[wiki/Systems/AI & Agentic Systems/Least-Cost Interpretation|build the version that is fastest to build]] — whenever nothing is pushing against it. It optimizes toward what a check can see rather than what the check stands for. And it produces specifics at its best effort in the register it uses when correct, so confidence grades nothing. Sorting the correction before answering it is the whole method, because each repair fails in a way particular to the class it was aimed at: a note telling the model to remember does nothing about a memory it does not have, a new rule does nothing about a rule already being satisfied without being honored, and a second pass is the only thing that catches what a first pass produces.
...
The scope is work where a decision made today binds output produced next week: serialized writing, canon-bearing documentation, a codebase with conventions nobody wants to restate. Only the first class needs accumulated state; the other three fire inside a single session, and three of the record's heaviest classes are three instances in one day. What the horizon decides is whether the apparatus pays for itself, and on a one-off piece the checks cost more than the corrections they prevent. The division of labor this sits inside — what the model is handed, what the human keeps — is [[wiki/Systems/AI & Agentic Systems/Writing with a Structure Engine|Writing with a Structure Engine]]; the classes are the triage layer under it.
...
A rule derived from accepted and rejected outputs binds where a rule minted from a complaint's surface does not, and where the two disagree the exemplars win — on this record by standing order rather than by measurement, since the exit audit's final test is a comparison against the accepted exchanges, and a render failing it fails whatever rules it passes. That is where the two-way test comes from: a candidate rule must **fail** the output that was rejected and **pass** the examples that were accepted, and a rule that cannot do both does not bind.
...
Optimizing toward what a check can see has no such curve under it. A check that asks for an assertion is satisfiable by any model that can write the assertion, and capability changes how convincingly it is written rather than whether. That is the class to watch, and the one where an error in this classification would surface first.
