#!/usr/bin/env node
/**
 * update-notes-catalog.mjs — regenerates notes/catalog.md (agent-only, gitignored).
 *
 * The public Notes page renders notes/index.md (hand-curated Condensed + Hubs only).
 * This script writes the full wiki inventory to notes/catalog.md on predev/prebuild.
 */
...
const CATALOG = join(ROOT, 'notes', 'catalog.md');
...
function catalogDocument() {
  ...
  const head = `---
title: "Wiki Catalog"
description: "Full wiki inventory for agents — gitignored, not published. Regenerated by scripts/update-notes-catalog.mjs."
type: system
created: 2026-05-02
updated: ${today}
tags:
...
Agent-only full inventory. \`notes/catalog.md\` is gitignored and never published. Humans use [[notes/index|Knowledge Base Index]]; agents read this file for exhaustive link, summary, and type coverage. Regenerated on \`npm run dev\` / \`npm run build\` — do not edit the tables by hand.

`;
  return head + buildCatalogTables();
}
...
function main() {
  ...
  let prev = '';
  try {
    prev = readFileSync(CATALOG, 'utf8');
  } catch {
    /* first run */
  }
  if (next !== prev) {
    writeFileSync(CATALOG, next);
    console.log('update-notes-catalog: regenerated notes/catalog.md');
  } else {
