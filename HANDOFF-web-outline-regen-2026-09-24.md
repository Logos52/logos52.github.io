...
2. The page is a regeneration from the owner's research notes, not a rewrite of anything. What you may read: the page as it stands (frontmatter, H1, kept blocks); the research notes under `research-notes/` (raw sources, workbench banks, the research folder) and the research banks under `wiki/Research/`, found with `grep -ril` on the subject's words and through `research-notes/raw/Source Index.md`; the URLs in the page's Sources block, fetched with WebFetch, at most six per page, skipping x.com, twitter.com and youtube.com, which will not load; the frontmatter and kept blocks of neighbouring pages, to see how a page links. Your own knowledge of the subject fills a gap only, where it is general and well established; it never carries the page. Extract facts from a note into a bullet list first and write from the list. Never carry a note's sentence onto a page. A research bank or workbench file may contain sentences a model wrote for a page; they are notes to you, never lines to keep.
3. Facts about the owner's own setup, rulings, bots, projects, numbers or history come only from the kept blocks, from the research notes, or from section 8 of this file. Do not invent any. A page whose notes give too little stays short; record what is missing in your final report.
...
2. Gather facts from the notes. Look the page up in `research-notes/raw/Source Index.md` (the compile-targets column names the page). Then `grep -ril "<two or three subject words>" research-notes wiki/Research --include=*.md`, and read what matches for facts only. WebFetch up to six Sources URLs where the subject is a product, a documented method or a study. Own knowledge only for a gap.
...
Git: after every eight to ten finished pages, `git add` those page files only (never `git add -A`, never anything under `research-notes/`), commit with a one-line message such as `Outline regen: <folder>, <n> pages`, then `git pull --rebase` and `git push origin outline-regen-2026-09-24`. On the web, origin is the private repo. On a rebase conflict in a wiki page, keep the version already on the remote and skip that page. Append one line to `log.md` per batch, in the file's own format: `## [2026-09-24] compile | Outline regen: batch <X>, <n> pages`.

Moving pages from the private repo to the public branch is the local session's job, or the owner's: on the public clone, `git fetch private outline-regen-2026-09-24`, then `git checkout private/outline-regen-2026-09-24 -- wiki log.md`, then commit and push to the public branch. Only `wiki/` and `log.md` cross over.
